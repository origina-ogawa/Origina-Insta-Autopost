...
方式: HTMLテンプレート + Gemini Flash(文章生成) + Playwright(画像化) + Instagram公式API + GitHub Actions(平日12:30お昼休み前後に実行)。
...
- **作業を始める前に必ず 進捗.md を読み、残タスクと直前の状況を把握する。** 作業の節目(タスク完了、方針決定、コミット・PR作成など)で必ず 進捗.md を更新する。自動圧縮でスレッドの記憶が失われても、このファイルを読めば状況を復元できるようにする
...
- **固定ブランドスライド**: カルーセルの末尾に「AI & DX支援ならオリジナ」+ロゴの固定スライドを自動付与(assets/logo-origina.png とconfig/brand.*.json で管理)
